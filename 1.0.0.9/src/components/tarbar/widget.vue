	...
	<div class="TyeArea">
		<div id="header">
			...
			<div class="Searchbox2"></div>
				...
				<audio  src="http://localhost:8080/images/11.mp3" id="audio" ></audio>
				<!-- 转盘 -->
				<div class="Turntable">
				<div class="container">
...
            	urls:'',
		...
		mounted(){
			...
			$(function(){
				...
				function gogo(){
					lisn = 0;
					$('.img').attr('src',that.pic[that.Trc-1]);
				    // url = audio.currentSrc//这里存起上一首的路径
						if(audio.src == that.going[that.going.length-1]){
							// console.log('与上一首相同的歌曲路径才不给你，免得污染')//应该多添加一个查询数组去除上上首之后的数组
						}else{
							that.going.push(audio.src);
						}
	...
	                    cd();	
				...
				//上一首
				 ...
				 function toto(){
					 ...
					 $('.img').attr('src',that.pic[that.Trc+1]);
					 if(lisn == 1){
						 audio.src = that.going[that.going.length-1];
					 }else{
						 if(that.going.length<lisn){
							 // alert('再没有上一首啦');
							  audio.src = that.going[0];
						 }else{
							 audio.src = that.going[that.going.length-lisn]
						 }
						 
					 }
					 
					 //  判断的是否URL 路径
					 // console.log(that.going.length);
					 // console.log(that.going[0])
					 // audio.src = that.going[that.going.length-1];
					 cd();
					  // if(urlcz == undefined){
					  //     return 
			  ...
			  var str = '';
			   ...
			   var w = audio.currentTime / audio.duration * 100 + "%";
				 ...
				 let times = formatTime(audio.currentTime);
				 var lll = Math.round(audio.currentTime)
				 if(lll == that.lrcs.lrctime[str]){
				 }
				 $('.musictext').scrollTop(list);
				  str++;
				  if(str == that.lrcs.lrctime.length){
				  }
